...
import type { LedgerEvent } from "@/server/employee/ledger";
...
import { WalletLedger } from "@/app/(app)/submit/wallet-ledger";

type View = "claims" | "ledger";

// KAN-146 — a lightweight two-way tab, not a full status-tab bar (there's no
// "Drafts"/"Pending"/etc. split in the real app today, so we don't invent one
// just to host "Ledger" — see the story's scoping note).
export function MyClaims({ claims, ledger, fy }: { claims: MyClaim[]; ledger: LedgerEvent[]; fy: string }) {
  ...
  return (
    <div className="max-w-[760px]">
      <h2 className="text-[17px] font-semibold tracking-[-0.01em]">
        {view === "claims" ? "My expense claims" : "Wallet ledger"}
      </h2>
      <p className="mt-1 mb-3.5 text-[13px] text-muted-foreground">
        {view === "claims"
          ...
          : "Every credit, debit, and hold across this financial year, with a running balance."}
      </p>

      <Segmented
        ariaLabel="My claims view"
        value={view}
        onChange={setView}
        options={[
          { value: "claims", label: "My expense claims" },
      ...
      {view === "ledger" ? (
        <WalletLedger events={ledger} fy={fy} />
      ) : (
        <>

...
- **Notification Preferences** (KAN-168, Employee Productivity Enhancements epic KAN-165) added `notificationPreferences` — a PER-USER row (unlike every config table above), lazily defaulted via `getNotificationPreferences` (`src/server/notifications/preferences.ts`). `src/app/(app)/settings/notifications/` is the first PERSONAL (non-HR/Admin) settings screen — any role edits their own row, gated by `requireUser()` alone (no `assertCan`), listed `ALL_ROLES` in `MODULE_ACCESS` for clarity. `isNotificationAllowed(userId, { channel })` gates every existing `sendEmail()` call site (`decide-expense.ts`, `leave.ts`, `approve-leave.ts`, `cancel-approved-leave.ts`, `actions/reminder-settings.ts`, `manager/capacity-alert.ts`) against the recipient's `emailEnabled` flag and IST quiet-hours window (`src/lib/quiet-hours.ts`, pure/dependency-free, handles a midnight-wrapping window). A gated/skipped send writes no `emailLog` row. `pushEnabled`/`inAppEnabled` are RECORDED ONLY — there is no push delivery (no web-push/service worker/VAPID) or in-app notification center yet; don't mistake the toggles for a shipped delivery mechanism.
- **Leave Balance History** (KAN-167, Employee Productivity Enhancements epic) is a DERIVED per-leave-type ledger, same "don't persist what can be computed" convention as KAN-146's wallet ledger: `getLeaveBalanceHistory` (`src/server/employee/leave-ledger.ts`) replays one synthetic opening-balance row per `leaveTypes` row (dated at FY start) plus every `accrue_leave_balance`/`deduct_leave_balance`/`restore_leave_balance` `auditLog` row for that user+FY's `leaveBalances.id`, into an independent running balance per leave type. Because a brand-new user's balance is seeded straight to `maxBalanceDays` (`ensureLeaveBalances` in `current-user.ts`), bypassing the audit trail, the derived total usually won't match the real stored `leaveBalances.balanceDays` — so one closing "adjustment" row per type (dated at the later of FY start or the last audit event) always reconciles the gap; a real deduction/restore/accrual event never gets silently dropped. Surfaced as a "Balance history" view alongside `MyRequests` on `/leave` (`leave-history.tsx`), same toggle + CSV-export-as-client-Blob pattern as the wallet ledger. No carry-forward rows yet — `carryForwardBalance` (`src/server/leave/accrual.ts`) is still never called anywhere.
...
- **Public Holiday Countdown** (KAN-206, part of the KAN-204 "Team Collaboration Enhancements" epic) added `users.location` (nullable text — unset means org-wide only) and a new read path, `getNextHoliday` (`src/server/employee/holiday-countdown.ts`), that queries the real `holidays` table directly (`id`/`date`/`name`/`location`) for the next upcoming date where `location IS NULL OR location = user.location`. This deliberately does NOT rewire `src/lib/working-days.ts`'s pre-existing hardcoded holiday map — that gap stays a separate follow-up. Surfaced as a standalone card (`dashboard/holiday-countdown-widget.tsx`) above the reminder banner on `/dashboard`; the countdown is plain server-rendered math off `todayISO()`, recomputed on every page load, no client ticking needed.
